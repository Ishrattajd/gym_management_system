...
const MemberDashboard = () => {
  ...
  const fetchDashboard = async () => {
    try {
      ...
      if (Array.isArray(attendanceData)) {
        // Filter only this member's attendance and sort by date descending
        const memberAttendance = attendanceData
          .filter(att => att.user_name === username)
      ...
      if (member) setMemberProfile(member);
  ...
  const recentAttendance = attendance.slice(0, 3); // latest 3 records
    ...
    <div className="container-fluid p-0 bg-dark-custom min-vh-100 d-flex">
      ...
      <main className="main-content">
        <header className="mb-5">
          ...
          <p className="text-muted fs-5">Here's your fitness overview</p>
        </header>
        ...
        <div className="row g-4 mb-5">
          <div className="col-md-3">
            <div className="card h-100 p-4 bg-card-custom stat-card-featured">
              ...
              <h3 className="text-white fw-bold m-0">{sessionsThisWeek} Sessions</h3>
            </div>
          </div>
          ...
          <div className="col-md-3">
            <div className="card h-100 p-4 bg-card-custom border-gray-custom">
              ...
              <h3 className="text-white fw-bold m-0">{upcomingClasses.length} Classes</h3>
            </div>
          </div>
          ...
          <div className="col-md-3">
            <div className="card h-100 p-4 bg-card-custom border-gray-custom">
              ...
              <h3 className="text-white fw-bold m-0">{memberProfile?.plan_name || "None"}</h3>
            </div>
          </div>
          ...
          <div className="col-md-3">
            <div className="card h-100 p-4 bg-card-custom border-gray-custom">
              ...
              <h3 className="text-white fw-bold m-0">{memberProfile?.goal || "Not Set"}</h3>
            </div>
          </div>
        </div>
        ...
        <div className="row g-4">
          ...
          <div className="col-md-6">
            <div className="card p-4 bg-card-custom border-gray-custom">
              <h5 className="text-white text-uppercase fw-bold mb-4">Upcoming Classes</h5>
              <div className="d-flex flex-column gap-3">
                {upcomingClasses.map((cls) => (
                  <div key={cls.id} className="p-4 rounded-3 bg-item-custom border-gray-custom d-flex justify-content-between align-items-center">
                    <div>
                      <h5 className="text-white fw-bold mb-1">{cls.class_type}</h5>
                      <p className="text-muted small mb-0">{cls.date}</p>
                    </div>
          ...
          <div className="col-md-6">
            <div className="card p-4 bg-card-custom border-gray-custom">
              <h5 className="text-white text-uppercase fw-bold mb-4">Recent Attendance</h5>
              <div className="d-flex flex-column gap-3">
                {recentAttendance.length === 0 && (
                  <p className="text-muted">No attendance recorded yet.</p>
                )}
                {recentAttendance.map((att) => (
                  <div key={att.id} className="p-4 rounded-3 bg-item-custom border-gray-custom d-flex justify-content-between">
                    <div>
                      <h5 className="text-white fw-bold mb-1">{att.date}</h5>
                      <p className="text-muted small mb-0">Status: {att.status}</p>
                    </div>
                    <span className={att.status?.toLowerCase() === "present" ? "text-success fw-bold" : "text-danger fw-bold"}>
                      {att.status}
                    </span>
                  </div>
                ))}
